...
- **Diárias concluídas na semana**: só as **diárias** com estado **concluído** deixam de aparecer nas colunas e são listadas num painel **abaixo** da grelha (canto à direita em LTR), com **data** e os mesmos controlos (⋮, lápis, ✓). **Todos** concluídas **permanecem** no calendário no dia de vencimento. Cada diária entra **uma vez** na lista (primeiro dia da semana em que entra na vista)
...
- Títulos dos botões e textos traduzíveis (`weekCalendar*`, `weekCalendarDragHandle`, `weekCalendarNowLine`, `weekCalendarCompletedTitle`, etc.); **não** há parágrafo de dicas longas abaixo do calendário (só o painel opcional de **diárias** concluídas, quando houver)
